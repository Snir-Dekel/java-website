...
public class RestoreAccount extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (!Functions.link_valid(Functions.get_email_from_deleted_users(verification_code), "restore")) {
            ...
            out.print("\n" +
                    ...
                    "    <a style=\"font-size: 200%\" href=\"restore_account.jsp\">Restore Account</a>\n" +
                    "\n" +
                    "    </div>\n" +
                    "\n" +
        ...
        if (Functions.get_verification_code(email) != null) {
            if (!Functions.get_verification_code(email).equals(verification_code)) {
                out.println("<HTML><HEAD><TITLE>● Restore Account Failed</TITLE>" + "<link rel=icon type=image/png href=logo.png>" +
                        "</HEAD><BODY><font size=50px>" + "Restore Account Failed</BODY></HTML>");
                return;
            }
        }
        ...
        try {
            String sql_query = "INSERT INTO users SELECT * FROM deleted_users WHERE email=?";
            ...
            if (preparedStatement.executeUpdate() <= 0) {
                throw new Exception("cant insert data to users table from deleted_users table");
            }
            sql_query = "DELETE FROM deleted_users WHERE email=?";
            preparedStatement = connection.prepareStatement(sql_query);
            preparedStatement.setString(1, email);
            if (preparedStatement.executeUpdate() <= 0) {
            ...
            e.printStackTrace();
            out.println("<HTML><HEAD><TITLE>● Restore Account</TITLE>" + "<link rel=icon type=image/png href=logo.png>" +
                    "</HEAD><BODY><font size=50px>This Account Is Not Deleted Or Had Never Existed</BODY></HTML>");
        } finally {
